...
export default {
  ...
  methods: {
    ...
    getWxMenu() {
       axios({
           url:'https://ygbao.magicxhx.com/api/coupon/getAllMenu.html',
           method: 'get',
           }).then(response =>{
    ...
    saveFun(){
      axios.post('https://ygbao.magicxhx.com/api/coupon/createMenu', this.menu).then(response => {
        if(response.data.errcode == 0) {
          ...
          this.$message({
            showClose: true,
            message: '注意：配置有错,微信服务器报错：' + response.data.errmsg + '',
            type: 'error',
            duration:10000
          })
        }
...
.public-account-management{
  ...
  .left{
    ...
    width: 299px;
    background: url("~@/assets/images/iphone-1.jpg") no-repeat;
    background-size: 100% auto;
    padding: 503px 25px 88px;
    position: relative;
